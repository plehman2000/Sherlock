...
# 1. pip install flask
# 2. Windows version:
# 2. set FLASK_APP=sherlock.py
# 2. Linux version:
# 2. export FLASK_APP=sherlock.py


# SET FLASK ENV VARIABLE FOR TEST RUN:
# $env:FLASK_APP = "sherlock.py"
...
def createEntityDefinitions(ents):
...
def getLinkFromTS(url, ts):
    vid_id = getIDFromURL(url)
    base_ts_url = f"https://youtu.be/{vid_id}?t="

    return base_ts_url + str(round(float(ts)))
...
def sherlock():
    ...
    if request.method == 'POST':
        print(str(request.form['sherlock_name']))

        #summary = 'test'
        # summary of video
        link = 'https://www.youtube.com/watch?v=br0e-strgZU&t=30s'
        link = str(request.form['sherlock_name'])
        ...
        if (transcript != -1):
            ...
            entDict = createEntityDefinitions(ents)
            dictObj = getEntTsPairs(ents)

            """
            for value in dictObj.values():
                for ts in value:
                    linkList.append(getLinkFromTS(link, ts))
            print(linkList)
            """
            for value in dictObj.values():
                linkList.append(getLinkFromTS(link, value[0]))

            print(linkList)
        
            for index, x in enumerate(dictObj.keys()):
                print(x)
                entList.append([x, dictObj[x], entDict[x], linkList[index]])
        ...
        #print(summary)
        test_var = str(request.form['sherlock_name'])




    return render_template('Home.html', test_var=test_var, summary = summary, entList = entList)
